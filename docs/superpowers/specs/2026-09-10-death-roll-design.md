...
- **Win**: `+ante`, whatever you spent on passes. Your own pass comes back to
  you inside the pot.
- **Lose**: `-(ante + your passes)`.

So the two nets sum to zero, always, and **a pass is only ever paid for by the
loser**. That is the right shape: passing is not a fee, it is a bet that you
will not be the one paying.
